...
------------------------------------------------------------
1. TEMPO / TIME / PULSES_PER_BEAT がすべて定義されている。
2. 各トラックの合計ステップが 1小節の整数倍になっている。
3. 不要な全角スペースやカンマが紛れ込んでいない。
4. CHECK ブロックがファイル末尾にあり、値が譜面内容と一致している。
...
- CHECK ブロックは必ずファイルの最後に置く。
- トラック名は譜面内のトラックと一致すること。
- 数値は、各トラックの合計ステップ数を示す（Bars_Total は小節数）。
- アプリは CHECK〜ENDCHECK のブロックを完全に無視する。
- これは教材・サンプル制作時のミス防止のために必須とする。
